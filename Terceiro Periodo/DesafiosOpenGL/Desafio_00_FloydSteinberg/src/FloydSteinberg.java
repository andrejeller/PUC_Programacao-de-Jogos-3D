...
public class FloydSteinberg {
    ...
    public BufferedImage converter(BufferedImage img){
        ...
        for (int y = 0; y < out.getHeight(); y++){
            for (int x =0; x < out.getWidth(); x++){
                ...
                int errorR = pixel.getRed() - newPixel.getRed();
                int errorG = pixel.getGreen() - newPixel.getGreen();
                int errorB = pixel.getBlue() - newPixel.getBlue();

                ////Aplicar para cada cor....

                int error = pixel.getRGB() - newPixel.getRGB();

                if( x + 1 < out.getWidth())
                    out.setRGB(x +1, y, error * 7/16);
                    //sb[ci  + 1] += (err*7)>>4;  // if right neighbour exists
                if(y + 1 == out.getHeight())
                    continue;   // if we are in the last line
                if(x > 0)
                    out.setRGB(x - 1, y + 1, error * 3/16);
                    //sb[ci+w-1] += (err*3)>>4;  // bottom left neighbour
                //sb[ci + w ] += (err*5)>>4;  // bottom neighbour
                if(x + 1 < out.getWidth())
                    out.setRGB(x + 1, y + 1, error * 7/16);
                    //sb[ ci + w + 1] += (err*1)>>4;  // bottom right neighbour



                out.setRGB(x, y, error);
    ...
    public void run() throws IOException {
        File PATH = new File ("C:\\Users\\andrejeller\\Desktop\\PUC\\Programação 3D\\img\\gray");
        BufferedImage img = ImageIO.read(new File(PATH, "lizard.png"));

        BufferedImage newImg = converter(img);

        ImageIO.write(newImg, "png", new File(PATH, "fsLizard.png"));
    }


    public static void main(String args[]) throws IOException {
        new FloydSteinberg().run();
    }
}